...
export const getApplicationExpressVersionESM = () => {
  ...
  try {
    const expressPackageJsonPath = require.resolve(
    ...
    if (packageJson.name === "express") {
      expressVersion = packageJson.version;
    }
  } catch (error) {
    console.warn(
      `[SDK] Could not determine application's Express.js version: ${error.message}`,
    );
    expressVersion = "Unknown or not found";
  }
